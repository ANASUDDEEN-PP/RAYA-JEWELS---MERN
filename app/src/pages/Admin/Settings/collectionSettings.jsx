...
import { ArrowLeft, User, Upload, Calendar, Loader2, Edit, Trash, Save, X } from 'lucide-react';
...
export default function CreateCollectionPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${baseUrl}/collection/create`, collectionData);
    ...
    const handleSaveEdit = async (id) => {
        try {
            ...
            const response = await axios.put(`${baseUrl}/collection/update/${id}`, editFormData);
            console.log(response)
            if (response.status === 200) {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this collection?')) {
            try {
                const response = await axios.delete(`${baseUrl}/collection/delete/${id}`);
                if (response.status === 200) {
                        ...
                        <div className="max-w-4xl mx-auto">
                            ...
                            <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border border-gray-100 mb-12">
                                <div className="space-y-6">
                                    {/* Collection Name */}
                                    <div>
                                        <label htmlFor="collectionName" className="block text-sm font-semibold text-gray-700 mb-2">
                                            Collection Name *
                                        </label>
                                        <input
                                            type="text"
                                    ...
                                    {/* Created Date */}
                                    <div>
                                        <label htmlFor="createdDate" className="block text-sm font-semibold text-gray-700 mb-2">
                                            Creation Date *
                                        </label>
                                    ...
                                    <button
                                        ...
                                        className={`w-full bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-semibold py-3 px-6 rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all duration-200 ${isLoading || !collectionName || !createdDate ? 'opacity-70 cursor-not-allowed' : ''
                            ...
                            <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-100">
                                <h2 className="text-xl font-bold text-gray-800 mb-6">Your Collections</h2>
                                
                                {collections.length === 0 ? (
                                    <p className="text-gray-500 text-center py-8">No collections found. Create one above!</p>
                                ) : (
                                                    ...
                                                    <tr key={collection._id}>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {editingId === collection._id ? (
                                                                ...
                                                                <div className="text-sm font-medium text-gray-900">{collection.CollectionName}</div>
                                                            )}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            {editingId === collection._id ? (
                                                                ...
                                                                <div className="text-sm text-gray-500">
                                                                    {new Date(collection.CreatedData).toLocaleDateString()}
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                            {editingId === collection._id ? (
                                                                <div className="flex space-x-2">
                                                                    <button
                                                                        onClick={() => handleSaveEdit(collection._id)}
